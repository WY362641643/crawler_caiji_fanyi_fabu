...
class MySQL_My(object):
    ...
    def run(self):
        ...
        try:
            ...
            # 查询 网站管理
            sql = "select id, isActivate, `colum`, author, weblink,`describe` from webnumber"
            cursor.execute(sql)  # 执行
            result = cursor.fetchall()  # result是元
            web_main = {}
            for item in result:
                web_main[item[0]] = item
            # 查询 采集错误url链接
            sql = "select id,title,url,web_id,ids from errmainurl"
            cursor.execute(sql)
            result = cursor.fetchall()
            data = []
            for item in result:
                data.append((web_main[item[3]], item))
    ...
    def updateDate(self, id, url_id):
        try:
            conn = pymysql.connect(self.host, self.user, self.pwd, self.dbname,port=self.port)  # 连接数据库
            cursor = conn.cursor()  # 获取游标
            sql = 'DELETE FROM errmainurl WHERE id=%d' % id
            cursor.execute(sql)
            sql = "update mainurl set isActivate=1 where id=%d" % url_id
            cursor.execute(sql)
            conn.commit()
            sql = 'select web_id from mainurl where id=%d'%url_id
            cursor.execute(sql)  # 执行
            result = cursor.fetchall()[0][0]  # result是元
            sql = 'update webnumber set numb = numb+1 where id=%d'%result
            cursor.execute(sql)
            conn.commit()
        except Exception as e:
            print('删除数据失败: {}, id为: {}'.format(e, id))
        finally:
            cursor.close()  # 关闭游标
            conn.close()  # 关闭连接
...
class MainCrawler(object):
    ...
    def run(self):
        ''''''
        key_url_list = MySQL_My().run()
        if key_url_list:
            for item in key_url_list:
                self.key_url_query.put(item)
                # self.crawlers(item[0], item[1])

    def crawlers(self):
        '''
        爬虫函数
        :param data_tupe: id, isActivate, `column`, author, weblink,`describe`
        :param data:id, title, url, web_id
        :return:
        '''
        while True:
            # try:
                data_tupe, data = self.key_url_query.get(block=True, timeout=60)
                print('爬虫函数', data)
                try:
                ...
                if res.status_code == 200:
                    ...
                    for xpa in self.xpathcont[webdoma].split(','):
                        ...
                        if not content_ls:
                            ...
                            break
                    if content_ls:
                        self.content_query.put(
                            [data[0], data_tupe[1], data_tupe[-1], data_tupe[4], data_tupe[3], data_tupe[2], data[1],
                             data[-1], content_ls])
                time.sleep(1)
            # except
            #     pass
    ...
    def formattedText(self):
        while True:
            try:
                data = self.content_query.get(block=True, timeout=60)
                print('整合文章', data[0:5])
                content = []
                sentence = ''  # 整合的句子小于800
                for texts in data[-1]:
                    ...
                    if len(str(texts)) > 800:
                        ...
                        for i, text_s in enumerate(text_ls):
                            if len(text_s) + len(sentence) < 800:
                                ...
                                content.append('\r\n\t' + sentence)
                                sentence = ''  # 整合的句子小于800
                    elif len(str(texts)) + len(sentence) < 800:
                        sentence += str(texts)
                    else:
                        # sentence 接近800字, 添加进列表,并分段,重置
                        content.append('\r\n\t' + sentence)
    ...
    def contnt_google(self):
        ...
        while True:
            try:
                data = self.formattedText_query.get(block=True, timeout=60)
                print('谷歌翻译文章', data[0:5])
                content = ''
                for texts in data[-1]:
                    content += '\r\n\t' + self.google_(texts)
                data[-1] = content
    ...
    def push_article(self):
        ...
        while True:
            try:
                id, isActivate, describe, weblink, author, column, title, url_id, content = self.content_google_query.get(
                    block=True, timeout=60)
                print('上传文章', id, isActivate, describe, weblink, author, column, title, url_id)
                if isActivate:
                    # 上传图片 API
                    add_article_url = '{}/apis.php?act=upload_Img'.format(weblink)
                else:
                    add_article_url = '{}/apis.php?act=article_Api'.format(weblink)
                # 织梦会话ID
                ...
                try:
                    cateid = int(random.choice(column.split(',')))
                except ValueError as e:
                    print(' 栏目id 发生错误,', column, e)
                    cateid = int(column)
                payload = {
                    "log_CateID": cateid,
                    "log_AuthorID": int(random.choice(author.split(','))),
                    "log_Tag": "",
                    "log_Status": 0,
                    "log_Type": 0,
                    "log_Alias": 0,
                ...
                response = requests.request("POST", add_article_url, data=payload, headers=headers)
                if "success" in response.text:
                    # if response.status_code == 200:
                    print("网站：%s，ID:%d，成功发布" % (title, id))
                    MySQL_My().updateDate(id, url_id)
            except:
                pass
            time.sleep(5)
    ...
    def work(self):
        ...
        for i in range(10):
            ...
            t.start()
        for i in range(8):
            t = Thread(target=self.formattedText)
            c_list.append(t)
            t.start()
        # 谷歌翻译进程
        # self.contnt_google()
