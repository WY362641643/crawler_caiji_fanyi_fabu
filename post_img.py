...
class img_upload():
    ...
    def upload_img(self):
        ...
        # filename = self.img_out(img_path1)  # 图片路径
        headers = {
            'Cookie': php_id,
        }
        payload = {
...
def sql_table_list(sql1, table):
    ...
    return a_list
# 为文章内容生成标签。
def content_to_html(content,background_link,phpid):
    # 开始提炼文章的关键词
    # text = html_to_content(content)
    # keyword_str = text_to_keywords(text)
    # 开始生成并上传第一张图片
    ac = img_upload(background_link,phpid)
    ...
    return content
# 开始上传文章。
def push_article(session_id, add_article_url, title, content, description, type_id):
...
# 文章上传管理程序。
def push_allin(session_id,title,content,description,background_link,type_id):
    add_article_url = background_link + "article_add.php"
    # article_id = db_article[0]
    # title = db_article[1]
    content = content_to_html(content,background_link,session_id)
    # description = db_article[3]
    try:
        status = push_article(session_id, add_article_url, title, content, description, type_id)
    except BaseException:
        pass
    # else:
    #     if status == "yes":
...
def upload_articles():
    # 获取所有先关网站信息
    # id_s = table[0]
    background_link = 'http://xitong.baobanzhang.com/ymhack2020/'
    username = 'admin'
    ...
    if session_id == "fail":
        ...
        sheet = 0
        content = '这是测试图片文章'
        push_allin(session_id,title,content,descrption,background_link,type_id)
