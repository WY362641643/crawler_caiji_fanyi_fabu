...
class Down_Compressimg(object):
    ...
    def compress_image(self,infile, quality=10):
        """不改变图片尺寸压缩到指定大小
        :param infile: 压缩源文件
...
def run():
    ...
    if key_url_list:
        ...
        for redis_name in key_url_list:
            for webid, img_isActivate, column, author, weblink, describe, isActivateTime, id, title, url, isActivate,ratioimg,cms in \
            key_url_list[redis_name]:
                key_url_query.put(
                    [webid, img_isActivate, column, author, weblink, describe, id, title, url, redis_name,
                     isActivateTime,ratioimg,cms])
        flag = True
    else:
    ...
    while True:
        ...
        try:
            if not key_url_query.empty():
                #  webid,img_isActivate,column,author,weblink,describe,id,title,url,redis_name,isActivateTime,ratioimg,cms= key_url_query.get(block=True,timeout=60)
    ...
    while True:
        ...
        try:
            if not formattedText_query.empty():
                # webid,img_isActivate,column,author,weblink,describe,id,title,url,redis_name,isActivateTime,ratioimg,cms,content = content_query.get(block=True,timeout=60)
                    ...
                    if data[1]:
                        ...
                        if randint_data <= int(data[11]):
                            # try:
                                ...
                                img_path = Down_Compressimg().run(data[7])
                                # 开始生成并上传第一张图片
                                img_src_1 = dedecms_upload_img(data[4],img_path, dedecms_session_ID[data[4]])
                                img_html = '<img src="%s" alt="%s">\n' % (img_src_1, data[7])
                                content += img_html
                            # except:
                            #     print('上传图片失败',get_nowtime())
