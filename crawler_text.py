...
class MySQL_My(object):
    ...
    def select_global(self):
        ...
        result = False
        try:
            sql = "select title,parameter,isActivate from globaldata"
            self.cursor.execute(sql)  # 执行
            result = self.cursor.fetchall()  # result是元
        finally:
            pass
            ...
            if self.r.llen('await:' + str(webname)):
                ...
                for item in range(20):
                    try:
                        ...
                        continue
                    if self.de_weight(webname, info[9]):
                        continue
                    all.append(info)
...
def content_delet_(content):
    content = re.sub(r'< a.*?>', '', content, re.S)  # 去除超链接
    content = re.sub(r'<span class="backword"><i class="backsohu"></i>返回搜狐，查看更多</span></a></p>','',content,re.S) # 去除搜狐广告
    content = re.sub(r'<div class="statement">声明：.*?/div>','',content,re.S) # 去除搜狐广告
    return content
...
def content_update_img(content, img_src):
    # 消除原来的图片
    img_src_p = re.compile(r'<img.*?>', re.S)
    imgs = img_src_p.findall(content)
    for i in imgs:
        content = content.replace(i, '')
    content += '<img src="{}" >'.format(img_src)
    return content
...
def formattedTextPool():
    ...
    '''
    while True:
        # # webid, img_isActivate, column, author, weblink, describe, id, title, url, redis_name, isActivateTime,
        # # ratioimg, cms, selecttitle, content= key_url_query.get(block=True, timeout=60)
        try:
            if not content_query.empty():
                ...
                # 判断文章字数是否达标
                if minNumberWordsisActivate and int(len(data[-1])) < int(minNumberWords):
                    print('字数未达标')
                    # return
                    continue
                elif len(data[-1]) > 800:
    ...
    while True:
        ...
        try:
            if not formattedText_query.empty():
                # webid,img_isActivate,column,author,weblink,describe,id,title,url,redis_name,isActivateTime,ratioimg,cms,selecttitle,content = content_query.get(block=True,timeout=60)
                data = formattedText_query.get()
                print('翻译文章ID：', data[0:8], get_nowtime())
                content = ''
                for texts in data[-1]:
                    content += google_(texts)
                content = C_trans_to_E(content)
                if not data[10]:
                ...
                if data[12] == '1':
                    ...
                    content = content_delet_(content)
                    payload = {
                    ...
                    payload = {
                        ...
                        "litpic": "",  # 缩略图片上传路径
                        "source": "",  # 文章来源名称
                        "writer": int(random.choice(data[3].split(','))),  # 作者
                        "typeid": int(random.choice(data[2].split(','))),  # 文章栏目
                        "typeid2": "",  # 文章副栏目
                        "keywords": "",  # 关键字9
