...
class User_Agent(object):
    ...
    def user_agent_data(self):
        data = {
            "browsers": {
                "chrome": [
                    "ozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/535.1 (KHTML, like Gecko) Chrome/14.0.835.163 Safari/535.1",
    ...
    if data[13] != '1':  # 查询是否打开了智能标题
        ...
        if str_count(data[7]) < 9:
            ...
            if data[13] == '4':
                ...
                htmlres = res.text
                related_title_list_str = p.findall(htmlres)
                related_title_list = re.findall('<a href="/s\?wd=.*?>(.*?)</a>', related_title_list_str[0], re.S)
                related_title = ''
                related_title_len = 0
