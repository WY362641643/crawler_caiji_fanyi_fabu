...
def get_phpid(background_link, user_name, password):
    try:
        ...
        if "成功登录" in code:
            ...
            print('智能采集 1767',"域名：%s，已经登录成功！" % (background_link))
            ...
            # create_str_to_txt(text)
            print('智能采集 1778',"域名：%s，登录失败,请检查网站后台链接和账号密码是否正确？" % (background_link))
            return False
    except BaseException:
        yun_times = time.strftime("%Y-%m-%d", time.localtime())
        text = "[{}]：网站：{}，登录失败,请检查网站网络是否出现问题。\n".format(yun_times, background_link)
        # create_str_to_txt(text)
        print('智能采集 1784',"域名：%s，登录失败,请检查网站后台链接和账号密码是否正确？" % (background_link))
        return False
...
def zblog_upload_img(url, img_path):
    ...
    if response_text == 'error':
        print('智能采集 1831',response_text)
        print('智能采集 1832','上传文件失败', get_nowtime())
    # else:
...
def run():
    ...
    try:
        ...
        if key_url_list:
            ...
            print('智能采集 1855','reids中没有 url链接', get_nowtime())
    except:
        pass


def crawlers():
    # '''爬虫函数''' # webid, img_isActivate, column, author, weblink, describe, id, title, redis_name,
    #                          isActivateTime, ratioimg, cms, selecttitle
    CollectingSource = [
    ...
    while True:
        ...
        try:
            if not key_url_query.empty():
                #  webid, img_isActivate, column, author, weblink, describe, id, title, redis_name,
                #  isActivateTime, ratioimg, cms, selecttitle= key_url_query.get(block=True, timeout=60)
                data = key_url_query.get()
                print('智能采集 1907','爬虫函数', data, get_nowtime())
            ...
            print('智能采集','url get请求,超时', data[2])
        except:
            if data:
                print('智能采集','爬虫函数发生错误', data[7])
    ...
    #                          isActivateTime, ratioimg, cms, selecttitle
        try:
            if not content_query.empty():
                ...
                # t_stop = time.time()
                # print('智能采集',"%s执行完成，耗时%.2f" % (os.getpid(), t_stop - t_start))
                # return data
        except:
            print('智能采集','%s进程发生错误' % os.getpid(), get_nowtime())
        time.sleep(2)
    ...
    #     data = ''
        try:
            if not formattedText_query.empty():
                ...
                content = re.sub('#', '</p><p>', content, flags=re.S)
                if not data[9]:
                    ...
                    if data[1]:
                        ...
                        if randint_data <= int(data[10]):
                            try:
                                ...
                                print('智能采集 2136','上传图片失败', get_nowtime())
                    payload = {
                        "channelid": 1,  # 默认值
                        "dopost": "save",
                        "shorttitle": "",  # 缩略标题
...
def push_article():
    ...
    while True:
            ...
            if not push_query.empty():
                ...
                if data[11] == '1':
                    ...
                    if "success" in response.text:
                        ...
                        print('智能采集',"网站：%s，文章发布失败: %s" % (data[8], data[7]), get_nowtime())
                        print('智能采集 2231',response.text)
        ...
        # time.sleep(5)
            ...
            # if not data:
            #     time.sleep(20)
            print('智能采集','上传文章函数发生错误', get_nowtime())
            # time.sleep(60)
        time.sleep(2)
