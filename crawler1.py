...
class Down_Compressimg(object):
    ...
    def compress_image(self, infile, quality=80):
        """不改变图片尺寸压缩到指定大小
        :param infile: 压缩源文件
...
class MainCrawler(object):
    ...
    def __init__(self):
        # self.author = MySQL_My().runAuthor()
        self.key_url_query = Queue()  # url 队列
        self.content_query = Queue()  # 网站抓取的文章 队列
        self.formattedText_query = Queue()  # 文章整合队列
        self.content_google_query = Queue()  # 谷歌翻译后的文章队列
        self.xpathcont = MySQL_My().xpath()
        self.webNameListRedis = Redis_my().webname()
        self.minNumberWords = 0
        self.minNumberWordsisActivate = False  # 检查文章最少字数 默认关闭
    ...
    def randomTimestamp(self, start, end, frmt='%Y-%m-%d %H:%M:%S'):
        return self.strTimeProp(start, end, random.random(), frmt)

    def run(self):
        ''''''
        key_url_list = Redis_my().select(self.webNameListRedis)
        if key_url_list:
            globalSttines = MySQL_My().select_global()
            if globalSttines:
                for item in globalSttines:
                    if item[2] and item[0] == '文章最少字数':
                        self.minNumberWords = item[1]
                        self.minNumberWordsisActivate = True

            for redis_name in key_url_list:
                for webid, img_isActivate, column, author, weblink, describe, isActivateTime, id, title, url, isActivate in \
                key_url_list[redis_name]:
                    self.key_url_query.put(
                        [webid, img_isActivate, column, author, weblink, describe, id, title, url, redis_name,
                         isActivateTime])
        else:
            print('reids中没有 url链接')
    ...
    def crawlers(self):
        ...
        while True:
            data = ''
            try:
                if not self.key_url_query.empty():
                    #  webid,img_isActivate,column,author,weblink,describe,id,title,url,redis_name,isActivateTime,= self.key_url_query.get(block=True,timeout=60)
                    data = self.key_url_query.get()
                    print('爬虫函数', data)
                    try:
                        ...
                        self.mainurlzeor(data[6])
                        continue
                    if res.status_code == 200:
                        webdoma = up.urlparse(data[8])[1]
                        html = res.text
                        html = etree.HTML(html)
                        for xpa in self.xpathcont[webdoma].split(','):
    ...
    def formattedText(self):
        ...
        while True:
            data = ''
            try:
                if not self.content_query.empty():
                    # webid,img_isActivate,column,author,weblink,describe,id,title,url,redis_name,isActivateTime,content = self.content_query.get(block=True,timeout=60)
                    data = self.content_query.get()
                    print('整合文章句子,ID：', data[0:8])
                    content = []
                    sentence = ''  # 整合的句子小于800
                    amount = 0  # 文字总量
                    for texts in data[-1]:
                    ...
                    amount += lens
                    # 判断文章字数是否达标
                    if self.minNumberWordsisActivate and int(amount) < int(self.minNumberWords):
                        return
                        # continue
                    data[-1] = content
                    self.formattedText_query.put(data)
                else:
                    print('规范函数为空')
            except requests.exceptions.ProxyError:
                print('代理IP错误 :', data[8])
                MySQL_My().updateDate(data[6], (data[7], data[8], data[0], data[6], '规范文章模块发生错误'))
            except:
                if data:
                    # print(data[8], '规范函数发生错误')
                    MySQL_My().updateDate(data[6], (data[7], data[8], data[0], data[6], '规范函数发生错误'))
                else:
                    print('规范函数为空')
                    time.sleep(10)
            time.sleep(0.5)

    def contnt_google(self):
        '''将爬虫获取到的content 通过谷歌翻译'''
        while True:
            data = ''
            try:
                if not self.formattedText_query.empty():
                    # webid,img_isActivate,column,author,weblink,describe,id,title,url,redis_name,isActivateTime,content = self.content_query.get(block=True,timeout=60)
                    data = self.formattedText_query.get()
                    print('翻译文章ID：', data[0:8])
                    content = ''
                    for texts in data[-1]:
                        content += '\r\n\t' + self.google_(texts)
                    if not data[-2]:
                        ...
                        random_time = self.randomTimestamp(start, end, frmt='%Y-%m-%d %H:%M:%S')
                    if data[1]:
                        # 下载图片,返回路径
                        ...
                        # 上传图片
                        try:
                            img_src = self.upload_img(add_article_url, img_path)
                    ...
                    data[-1] = payload
                    self.content_google_query.put(data)
                else:
                    print('谷歌翻译函数为空')
                time.sleep(3)
            except requests.exceptions.ProxyError:
                print('谷歌代理IP错误 :', data[8])
                MySQL_My().updateDate(data[6], (data[7], data[8], data[0], data[6], '谷歌翻译函数 代理 IP 错误'))
            except:
                if data:
                    print(data[5], '谷歌翻译函数发生错误')
     ...
     # 上传图片
    def upload_img(self, url, img_path):
    ...
    def push_article(self):
        ...
        while True:
            data = ''
            try:
                if not self.content_google_query.empty():
                    # webid, img_isActivate, column, author, weblink, describe, id, title, url, content_ls, redisName,content = self.content_query.get(block=True,timeout=60)
                    data = self.content_google_query.get()
                    print('上传文章:', data[0:11])
                    # if data[1]:
                    #     # 下载图片,返回路径
                    #     img_path = Down_Compressimg().run(data[7])
                    #     # 上传图片 API
    ...
    def work(self):
        ...
        c_list = []
        for i in range(5):
            t = Thread(target=self.crawlers)
            c_list.append(t)
            t.start()
        for i in range(5):
            p = Process(target=self.formattedText)
            c_list.append(p)
            p.start()
        for i in range(3):
            t = Thread(target=self.contnt_google)
            c_list.append(t)
            t.start()
        for i in range(2):
            t = Thread(target=self.push_article)
            c_list.append(t)
            t.start()
        time.sleep(5)
        while True:
            if self.content_query.empty() and self.key_url_query.empty() and self.formattedText_query.empty() and self.content_google_query.empty():
                self.run()
                print('\t\t\t\t\t\t\t\t\t\t获取url链接')
            time.sleep(10)
        # for p in c_list:
