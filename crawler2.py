...
def SQLupdateDate(id, args, webid=1):
    ...
    try:
        if not args:
            ...
            try:
                if 'l' in str(id):
                    ids = int(id[:-1])
                    sql = "update admin_lexiconurl set `date`='{}',`isActivate`=1 where id={}".format(date,ids)
                else:
                    sql = "update admin_mainurl set `date`='{}',`isActivate`=1 where id={}".format(date,id)
                cursor.execute(sql)
        ...
        print('修改数据库 ：{}，ID：{} webid:{} arges:{}'.format(e,id,webid,args))
    finally:
        cursor.close()  # 关闭游标
        conn.close()  # 关闭连接
...
def SQLmainurlzeor(id):
    conn = pymysql.connect(SQLhost, SQLuser, SQLpwd, SQLdbname, port=SQLPORT)  # 连接数据库
    cursor = conn.cursor()  # 获取游标
    try:
        if 'l' in str(id):
            ids = int(id[:-1])
            sql = "update admin_mainurl set isActivate=0  where id=%d" % ids
        else:
            sql = "update admin_mainurl set isActivate=0  where id=%d" % id
        cursor.execute(sql)
        conn.commit()
    except:
        print('SQLmainurlzeor 发生错误')
    finally:
        cursor.close()  # 关闭游标
        conn.close()  # 关闭连接
...
def RedisWebSiteOK(id,webid):
    ...
    if 'l' in str(id):
        id = int(id[:-1])
        Redisconn.rpush('have_published',(True,False,id,webid))
    else:
        Redisconn.rpush('have_published', (True, False, id, webid))
...
def Redisselect(webname_list):
    ...
    for webname in webname_list:
        # if webname != 'www.law6b.com':
        #     continue
        if Redisconn.llen('await:' + str(webname)):
            all = []
            for item in range(20):
                try:
                    info = eval(Redisconn.lpop('await:' + str(webname)))
                except:
                    continue
                if Redisde_weight(webname, info[9]):
                    continue
                all.append(info)
...
def Redisde_weight(webname, url):
    ...
    key = get_md5(url)
    value = Redisconn.hget(name='issue:' + str(webname), key=key)
    return value


def Redisset_hash(webname, url):
    # print('插入 redis 已发布成功的数据: webname:{}, url:{}'.format(webname,url))
    Redisconn.hset(name='issue:' + str(webname), key=get_md5(url), value=1)
...
class Down_Compressimg(object):
    ...
    def getPage(self, keyword, page=2):
        ...
        url = url_begin + keyword + '&pn=' + str(page) + '&gsm=' + str(
            hex(page)) + '&ct=&ic=0&lm=-1&width=0&height=0&z=2'
        return url

    def get_onepage_urls(self, onepageurl):
        try:
            html = requests.get(onepageurl, headers={'User-Agent': User_Agent().random()}, proxies=get_ip(), timeout=60).text
        except Exception as e:
            print(e)
            pic_url = []
            return pic_url
    ...
    def down_pic(self, pic_urls, title, quality=80):
        ...
        for i, pic_url, in enumerate(pic_urls):
            try:
                pic = requests.get(pic_url, headers={'User-Agent': random.choice(user_agent)}, timeout=60, proxies=get_ip())
...
def get_ip():
    try:
        ips = requests.get("http://kuyukuyu.com/agents/get?uuid=d5a51445-b1bc-4ecb-81df-db794d704cbb", timeout=60).text
    except:
        time.sleep(0.5)
        ips = get_ip()
    return {
...
# 过滤联系方式
def filter_contact(content_in):
    content_text = content_in.split('\n')
    text = ''
    res = ["1[3458]\\d{9}", "[1-9]\\d{4,10}"]
    for content_ in content_text:
        if content_:
            contact_text = ""
            # 开始正则过滤联系方式
            for re_ in res:
                af = re.findall(re_, content_)
                if af:
                    contact_text = af
            # 开始过滤敏感词
            sensitive_word = ['转载', '注明出处', '联系我们', '微信', '展开全部', '来源', '百度首页\n登录\n个人中心\n帐号设置\n意见反馈\n退出\n']
            for pattern in sensitive_word:
                match = re.search(pattern, content_)
                if match:
                    contact_text = pattern
            # 没有找到敏感词
            if contact_text:
                pass
            else:
...
globalSttines = SQLselect_global()
...
def dedecms_upload_img(url, filename, phpid):
    ...
    response = requests.request("POST", url, data=m, headers=headers, params=querystring, timeout=60)
...
def run():
    ...
    if key_url_list:
        for redis_name in key_url_list:
            for webid, img_isActivate, column, author, weblink, describe, isActivateTime, id, title, url, isActivate, ratioimg, cms, selecttitle in \
    ...
    while True:
        ...
        try:
            if not key_url_query.empty():
                #  webid,img_isActivate,column,author,weblink,describe,id,title,url,redis_name,isActivateTime,ratioimg,cms,selecttitle= key_url_query.get(block=True,timeout=60)
                ...
                if res.status_code == 200:
                    ...
                    if not content_ls:
                        ...
                        SQLupdateDate(data[6], (data[7], data[8], data[0], data[6], '未匹配到文章数据'))
                    else:
            ...
            SQLupdateDate(data[6], (data[7], data[8], data[0], data[6], 'url get请求,超时'))
        except:
            if data:
                print('爬虫函数发生错误', data[8])
                # SQLupdateDate(data[6], (data[7], data[8], data[0], data[6], '爬虫函数发生错误'))
            else:
                print('爬虫函数为空')
                time.sleep(2)
        time.sleep(3)
        ...
        try:
            if not content_query.empty():
                ...
                if data[13] != '1':  # 查询是否打开了智能标题
                    ...
                    if str_count(data[7]) < 9:
                        ...
                        if data[13] == '4':
                            ...
                            urls = 'https://www.baidu.com/s?wd={}'.format(keyword)
                            res = requests.get(urls, headers=headers,
                                               proxies=get_ip(), timeout=60)
                            p = re.compile('相关搜索.*?<a.*?>.*?</a>.*?</th></tr></table></div>', flags=re.S)
                            related_title_list_str = p.findall(res.text)[0]
                            ...
                            title += '(' + related_title + ')' + '!@#' + title
                        else:
                            urls = 'http://suggestion.baidu.com/su?wd={}&sugmode=3&json=1'.format(keyword)
                            res = requests.get(urls,headers={'User-Agent': User_Agent().random()},
                                               proxies=get_ip(), timeout=60)
                            html = res.text
                ...
                if len(conts) < 800:
                    ...
                    content_list.append(sentence)
                data.insert(14, related_title_list)
                data[-1] = content_list
                formattedText_query.put(data)
        except ImportError as e:
            print('%s发生错误' % os.getpid(), get_nowtime(),e)
        except requests.exceptions.ConnectionError:
            print('%s发生错误,请求超时' % os.getpid(), get_nowtime())
        time.sleep(2)
...
def contnt_google():
    while True:
        ...
        try:
            ...
            SQLupdateDate(data[6], (data[7], data[8], data[0], data[6], '谷歌翻译函数 代理 IP 错误'))
        # except:
        #     if data:
        #         print(data[:], '谷歌翻译函数发生错误', get_nowtime())
        #         SQLupdateDate(data[6], (data[7], data[8], data[0], data[6], '谷歌翻译函数发生错误'))
        #     else:
        #         print('谷歌翻译函数为空')
        #         time.sleep(2)
        time.sleep(2)
        ...
        try:
            if not push_query.empty():
                ...
                if data[12] == '1':
                    ...
                    response = requests.request("POST", add_article_url, data=data[-1], headers=headers, timeout=60)
                    if "success" in response.text:
                        # if response.status_code == 200:
                        print("网站：%s，成功发布文章：%s" % (data[9], data[7]), get_nowtime())
                        SQLupdateDate(data[6], False, data[0])
                        Redisset_hash(data[9], data[8])
                    else:
...
if __name__ == '__main__':
    ...
    #     with ThreadPoolExecutor(5) as executor:
    #         allp = [executor.submit(push_article, ) for i in range(zblog_push_query.qsize())]
    c1 = Thread(target=crawlers).start()
    time.sleep(1)
    c2 = Thread(target=crawlers).start()
    time.sleep(1)
    c3 = Thread(target=crawlers).start()
    time.sleep(1)
    c4 = Thread(target=crawlers).start()
    time.sleep(1)
    c5 = Thread(target=crawlers).start()
    time.sleep(1)
    f1 = Thread(target=formattedTextPool).start()
    time.sleep(1)
    f2 = Thread(target=formattedTextPool).start()
    time.sleep(1)
    f3 = Thread(target=formattedTextPool).start()
    time.sleep(1)
    t1 = Thread(target=contnt_google).start()
    time.sleep(1)
    t2 = Thread(target=contnt_google).start()
    time.sleep(1)
    t3 = Thread(target=contnt_google).start()
    time.sleep(1)
    t4 = Thread(target=contnt_google).start()
    time.sleep(1)
    t5 = Thread(target=contnt_google).start()
    time.sleep(1)
    t6 = Thread(target=contnt_google).start()
    time.sleep(1)
    t7 = Thread(target=contnt_google).start()
    time.sleep(1)
    t8 = Thread(target=contnt_google).start()
    time.sleep(1)
    t9 = Thread(target=contnt_google).start()
    time.sleep(1)
    t0 = Thread(target=contnt_google).start()
    time.sleep(1)
    p1 = Thread(target=push_article).start()
    time.sleep(1)
    p2 = Thread(target=push_article).start()
    time.sleep(1)
    p3 = Thread(target=push_article).start()
    time.sleep(1)
    p4 = Thread(target=push_article).start()
    time.sleep(1)
    p5 = Thread(target=push_article).start()
    time.sleep(1)
    p6 = Thread(target=push_article).start()
    time.sleep(1)
    p7 = Thread(target=push_article).start()
    time.sleep(1)
    p8 = Thread(target=push_article).start()
    time.sleep(5)
    while True:
        if content_query.empty() and key_url_query.empty() and formattedText_query.empty() and push_query.empty():
            run()
            print('\t\t\t\t\t\t\t\t\t\t获取url链接')
        else:
            print('管道中还有文章')
    ...
    # s = Redisde_weight('bxzphz.cn','https://baijiahao.baidu.com/s?id=1603170901791671174&wfr=spider&for=pc')
    # print(s)
    # strat = time.time()
    # print(strat)
    # SQLrun()
